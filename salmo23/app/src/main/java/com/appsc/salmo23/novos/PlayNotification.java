...
public class PlayNotification extends AppCompatActivity {
    ...
    private static String TAG = PlayNotification.class.getSimpleName();
    private String VIEW;
    private String OPEN;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (isEnglish()) {
            VIEW = "https://softcroy.com/app_santos_v2/api3/prayer/visualizacao_oracao_en.php?id=";
            OPEN = "https://softcroy.com/app_santos_v2/api3/notification/lac_open_oracoes_en.php?id=";
        } else {
            VIEW = "https://softcroy.com/app_santos_v2/api3/prayer/visualizacao_oracao_br.php?id=";
            OPEN = "https://softcroy.com/app_santos_v2/api3/notification/lac_open_oracoes_br.php?id=";
        }
        ...
        img = findViewById(R.id.img); // CORREÇÃO: Usando a ImageView para evitar ClassCastException

        configurarMusicaFundo();

        if (getSupportActionBar() != null) {
            TextView tv = new TextView(getApplicationContext());
            tv.setText(getIntent().getStringExtra("NOME_SEND"));
            tv.setTextSize(23);
        ...
        // Glide corrigido para carregar na ImageView
        String imageUrl = getIntent().getStringExtra("IMAGE_SEND");
        RequestOptions requestOptions = new RequestOptions().diskCacheStrategy(DiskCacheStrategy.ALL);
        Glide.with(this).load(imageUrl).apply(requestOptions).into(img);

        oracao.setText(getIntent().getStringExtra("ORACAO_SEND"));
        ...
        sha.setOnClickListener(v -> {
            ...
            shareIntent.putExtra(Intent.EXTRA_TEXT, getIntent().getStringExtra("ORACAO_SEND") + "  " + getString(R.string.app_name) + " " + "https://play.google.com/store/apps/details?id=" + getPackageName());
    ...
    private void playy1() {
        ...
        try {
            mPlayer.setDataSource(getIntent().getStringExtra("SOND_SEND"));
            mPlayer.prepareAsync();

            mPlayer.setOnPreparedListener(mp -> {
                // VERIFICAÇÃO CRUCIAL: Checar se mPlayer ainda existe antes de dar start
                if (mPlayer != null) {
            ...
            mPlayer.setOnCompletionListener(mp -> {
                ...
                if (bgMediaPlayer != null) bgMediaPlayer.setVolume(musicVolume, musicVolume);
                // Ao invés de anular aqui, apenas resetamos o estado visual
                // mPlayer = null; // Remova ou mova com cautela
            });
    ...
    public void onPause() {
        ...
        // Para a oração se o usuário sair da tela
        if (mPlayer != null) {
            try {
    ...
    // REMOVIDO: MobileAds.initialize (isso agora é gerenciado de forma centralizada)
    ...
    // Limpa o container e adiciona o novo espaço de anúncio
    ...
    private void CountVisualizacao() {
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.GET,
                VIEW + getIntent().getStringExtra("IDANTERIOR_SEND"), null, response -> {},
    ...
    private void CountOpen() {
        RequestQueue mRequestQueue = Volley.newRequestQueue(this);
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.GET,
                OPEN + getIntent().getStringExtra("ID_SEND"), null, response -> {},
                error -> VolleyLog.d(TAG, "Error: " + error.getMessage()));
        jsonObjReq.setRetryPolicy(new DefaultRetryPolicy(30000, 1, 1f));
        mRequestQueue.add(jsonObjReq);
    }
